	...
	class Update
	{
		...
		function Make16BitKey($Path)
		{
			...
			if($pmKeyLen)
			{
				...
				$md5Out = md5($Path);
				$md5Out = $this->hex2bin($md5Out);
				$first = ord($md5Out[15]);
				$limit = ord($md5Out[13]) * ord($md5Out[14]);
				$Path = substr($Path, ($first < $pmKeyLen ? $first : $first % $pmKeyLen),
					($limit < $pmKeyLen ? $limit : $limit % $pmKeyLen));
				//Secend mD5 from first MD5 of pre_master Key
				$decryptionKey = md5($Path);
				$decryptionKey = $this->hex2bin($decryptionKey);
