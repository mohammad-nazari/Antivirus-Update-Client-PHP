	...
	class AESClass
	{
		...
		function EncryptAESData($data)
		{
			...
			if ($encData != "")
			{
				...
				$encData = mcrypt_encrypt(MCRYPT_RIJNDAEL_256, $this->password, $encData, MCRYPT_MODE_ECB);
...
//				$decData = base64_decode(($decData));
				...
				$decData = mcrypt_decrypt(MCRYPT_RIJNDAEL_256, $this->password, $decData, MCRYPT_MODE_ECB);
				//Cut raw data from start to "\0"
				list($decData, $trashData) = explode("\0", $decData);
				return $decData;
			}
		...
		function PublicEncryptData($data)
		{
			...
			{
				//encrypt it
				openssl_public_encrypt($encData, $encData, $this->publicKey);
				return $encData;
			}
		...
		function PrivateDecryptData($data)
		{
			...
			if ($decData != "")
			{
				...
				$decData = '';
				//decrypt it
				openssl_private_decrypt($decData, $decData, $this->privateKey);
				//Cut raw data from start to "\0"
				list($decData, $trashData) = explode("\0", $decData);
...
//				$encData = base64_encode($encData);
				...
				$encData = $this->EncryptAESData($encData);
				//Now we will encrypt the password with the public key
//			$encData = $this->PublicEncryptData($encData);
